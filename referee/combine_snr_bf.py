...
def main():
    ...
    for run in all_runs:
        ...
        rows.append((run, lbf, opt_med, opt_lo, opt_hi, mf_med, mf_lo, mf_hi))
    ...
    header = f"{'Run':<{col}}  {bf_label:>10}  {snr_col_hdr:<26}  {'net SNR mf  (med [' + str(pct) + '% HDI])':26}"
    sep    = "-" * len(header)
    print(header)
    print(sep)
    for run, lbf, om, ol, oh, mm, ml, mh in rows:
        print(f"{run:<{col}}  {fb(lbf)}  {fs(om, ol, oh)}  {fs(mm, ml, mh)}")
    print(sep)
