"""
NOTE: has to be exectued on Jarvis!
"""
...
PRIOR_SAMPLE_SIZE = 300000   # Number of initial samples to draw from priors
OUTPUT_SAMPLE_SIZE = 30000  # Number of output samples after m1 >= m2 constraint

# KDE parameters
KDE_NBINS = 10000             # Number of points for KDE evaluation (higher = smoother curves)
KDE_SMOOTH = 2.0              # Smoothing factor (None = auto, or set to float like 1.5 for more smoothing)
# KDE_SMOOTH = None

# Prior domain configuration
PRIOR_DOMAIN_WIDE = (0.1, 6.0)  # Wide domain for KDE evaluation

# Output configuration
OUTPUT_PATH = 'populations_component_masses_comparison.pdf'
OUTPUT_PATH_WIDE = 'check_populations_component_masses_comparison_wide.pdf'
DPI = 600
...
def load_posterior_data(data_dir='/work/wouters/GW231109/', run_names=None, nsamp=OUTPUT_SAMPLE_SIZE):
    """
    ...
    for run_name in run_names:
        file_path = data_dir / f"{run_name}/outdir/final_result/GW231109_data0_1383609314-056813_analysis_H1L1_result.hdf5"
        ...
        samp = data['posterior']
        m1_data = np.random.choice(samp['mass_1_source'], nsamp, replace=True)
        m2_data = np.random.choice(samp['mass_2_source'], nsamp, replace=True)
...
def compute_kdes_batch(data_dict, Nbins=KDE_NBINS, smooth=KDE_SMOOTH, prior_domains=(1.0, 4.0), use_method_selection=False):
    ...
    for name, data in data_dict.items():
        ...
        print('Prior domains', xMin, xMax, Nbins)
        ...
        kde = kernel(np.linspace(xMin, xMax, Nbins))
        kde_dict[name] = {
...
def compute_js_on_common_grid(kde_dict1, kde_dict2, Nbins=KDE_NBINS):
    ...
    p1 = kde_dict1['kernel'](x_common)
    p2 = kde_dict2['kernel'](x_common)
...
def main():
    ...
    for name in m1_prior_data.keys():

        plt.hist(m1_prior_data[name], histtype='step', density=True)
        plt.plot(m1_prior_kdes[name]['x'], m1_prior_kdes[name]['kde'])
        plt.savefig('m1_{}.pdf'.format(name))
        plt.close()
    
        plt.hist(m2_prior_data[name], histtype='step', density=True)
        plt.plot(m2_prior_kdes[name]['x'], m2_prior_kdes[name]['kde'])
        plt.savefig('m2_{}.pdf'.format(name))
        plt.close()
    ...
    print('available distr', m1_posterior_kdes.keys(), m1_prior_kdes.keys())
    print('\n======== mass 1 JSD ========')
    ...
    for post_idx, post_name in enumerate(dist_names):
        ...
        for prior_idx, prior_name in enumerate(dist_names):
            jsd_val = compute_js(
                m1_posterior_kdes[post_name]['kde'],
                m1_prior_kdes[prior_name]['kde']
            )
    ...
    for post_idx, post_name in enumerate(dist_names):
        ...
        for prior_idx, prior_name in enumerate(dist_names):
            ...
            jsd_val = compute_js(
                m2_posterior_kdes[post_name]['kde'],
                m2_prior_kdes[prior_name]['kde']
            )
